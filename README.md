...
- **Lightning-fast indexing** — 递归遍历 + inotify 实时监听文件变更
- **No import/export** — 文件夹即图库，零迁移成本
- **Smart deduplication** — BLAKE3 精确去重 + DHash/PHash 感知去重
- **Screenshot detection** — 规则引擎自动识别截图
- **Map view** — Leaflet 地图视图，按地理位置浏览照片
...
| AI Extension | Python sidecar (optional, planned) |
