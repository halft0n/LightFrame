...
fn apply_writer_pragmas(conn: &Connection) -> lightframe_core::Result<()> {
    conn.execute_batch(
...
fn apply_reader_pragmas(conn: &Connection) -> lightframe_core::Result<()> {
    conn.execute_batch(
        "PRAGMA journal_mode = WAL;
...
impl Database {
    pub fn open(path: &Path) -> lightframe_core::Result<Self> {
        let writer =
            Connection::open(path).map_err(|e| lightframe_core::Error::Database(e.to_string()))?;
    ...
    pub fn open_default() -> lightframe_core::Result<Self> {
    ...
    pub fn conn(&self) -> lightframe_core::Result<std::sync::MutexGuard<'_, Connection>> {
        self.writer
            .lock()
            .map_err(|e| lightframe_core::Error::Other(format!("writer mutex: {e}")))
    }

    /// Read-only connection for queries (does not block on writer lock).
    pub fn read_conn(&self) -> lightframe_core::Result<std::sync::MutexGuard<'_, Connection>> {
        self.reader
            .lock()
            .map_err(|e| lightframe_core::Error::Other(format!("reader mutex: {e}")))
    }

    fn run_migrations(&self) -> lightframe_core::Result<()> {
        let conn = self.conn()?;
        migrations::run(&conn)
    }
}
