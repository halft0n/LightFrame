...
impl PythonSidecar {
    pub async fn spawn() -> Result<Self> {
        let python_dir = locate_python_dir().ok_or_else(|| {
            catchlight_core::Error::Ai(
                "Python sidecar package not found (set CATCHLIGHT_PYTHON_PATH)".into(),
            )
        })?;

        let mut child = tokio::process::Command::new("python3")
            .args(["-m", "catchlight_ai"])
    ...
    pub async fn call_with_timeout(
        ...
        let mut line = serde_json::to_string(&request).map_err(|e| {
            catchlight_core::Error::Ai(format!("failed to serialize JSON-RPC request: {e}"))
        })?;
        line.push('\n');
        ...
        let stdin = guard
            .stdin
            .as_mut()
            .ok_or_else(|| catchlight_core::Error::Ai("Python sidecar stdin unavailable".into()))?;
        stdin.write_all(line.as_bytes()).await?;
        stdin.flush().await?;

        let stdout = guard.stdout.as_mut().ok_or_else(|| {
            catchlight_core::Error::Ai("Python sidecar stdout unavailable".into())
        })?;
        ...
        let read_future = async {
            ...
            loop {
                ...
                if bytes == 0 {
                    return Err(catchlight_core::Error::Ai(
                ...
                let response: Value = serde_json::from_str(trimmed).map_err(|e| {
                    catchlight_core::Error::Ai(format!(
                ...
                if response.get("id").and_then(Value::as_u64) == Some(id) {
                    if let Some(err) = response.get("error") {
                        ...
                        return Err(catchlight_core::Error::Ai(format!(
                            "Python sidecar RPC error {code}: {message}"
                        )));
                    }
                    return response.get("result").cloned().ok_or_else(|| {
                        catchlight_core::Error::Ai("JSON-RPC response missing result field".into())
                    });
                }
            }
        };
...
fn locate_python_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("CATCHLIGHT_PYTHON_PATH") {
        let p = PathBuf::from(path);
        if p.join("catchlight_ai").is_dir() {
            return Some(p);
        }
    }
    ...
    if workspace_python.join("catchlight_ai").is_dir() {
    ...
    for _ in 0..6 {
        ...
        if candidate.join("catchlight_ai").is_dir() {
...
mod tests {
    ...
    fn locate_python_dir_finds_workspace_package() {
        ...
        assert!(
            dir.is_some(),
            "expected workspace python/catchlight_ai directory"
        );
        assert!(dir.unwrap().join("catchlight_ai").is_dir());
    }
