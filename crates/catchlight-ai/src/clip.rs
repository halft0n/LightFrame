...
impl ClipEncoder {
    pub fn new(model_path: &Path) -> Result<Self> {
        ...
        let session = Session::builder()
            .map_err(|e| catchlight_core::Error::Ai(format!("CLIP session builder: {e}")))?
    ...
    pub fn encode_image(&mut self, path: &Path) -> Result<Vec<f32>> {
        ...
        let outputs = self
            ...
            .run(ort::inputs![TensorRef::from_array_view(&tensor).map_err(
                |e| catchlight_core::Error::Ai(format!("CLIP tensor: {e}"))
            )?])
            .map_err(|e| catchlight_core::Error::Ai(format!("CLIP inference failed: {e}")))?;

        let output = outputs
            .values()
            .next()
            .ok_or_else(|| catchlight_core::Error::Ai("CLIP model returned no outputs".into()))?;

        let array = output
            .try_extract_array::<f32>()
            .map_err(|e| catchlight_core::Error::Ai(format!("CLIP output extraction: {e}")))?;
...
fn preprocess_image(path: &Path) -> Result<Array4<f32>> {
    let img = image::open(path)
        .map_err(|e| {
            catchlight_core::Error::Ai(format!("failed to open image {}: {e}", path.display()))
        })?
