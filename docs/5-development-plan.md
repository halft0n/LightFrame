...
| 交付物 | 验收标准 |
|--------|----------|
| 精确去重 | 10 万张 BLAKE3 < 5 分钟；重复组准确识别 |
| 感知去重 | 不同压缩率同图识别率 > 99%（测试集） |
| 截图识别 | 规则层召回 > 85%；CLIP 层准确率 > 90% |
| 地点+地图 | GPS 照片正确分组；地图钉点可点击 |
| 相簿/收藏/删除 | CRUD 完整；软删除 30 天清理 |
| FTS5 搜索 | 10 万库响应 < 50ms |

**Phase 2 完成标志**：差异化功能（去重、截图、搜索）可用，形成与 iPhotron/Lap 的差异。
