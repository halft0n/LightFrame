...
CatchLight 的 AI 功能在本地运行，不上传云端。需要 ONNX 模型文件：
...
3. 也可从 [GitHub Releases](https://github.com/halft0n/CatchLight/releases) 下载模型包
4. 点击 **刷新状态** 确认模型已识别
...
| 平台 | 路径 |
|------|------|
| Linux | `~/.local/share/catchlight/models/` |
| Windows | `%LOCALAPPDATA%\catchlight\models\` |
| macOS | `~/Library/Application Support/catchlight/models/` |
...
CatchLight 暂未购买代码签名证书。首次运行时点击「更多信息」→「仍要运行」。详见 [SIGNING.md](SIGNING.md)。
...
若出现「database locked」，关闭其他可能访问数据库的进程，重启 CatchLight。不要在外部工具中打开 `library.db`。
