...
- 文件名搜索（FTS5 全文）
- 日期搜索（如 `2024-01`）
- 语义搜索（需安装 CLIP 模型，搜索栏切换 AI 模式可用自然语言描述搜索图片内容）
- 搜索历史（最近 10 条）
...
| 模型 | 用途 | 约大小 |
|------|------|--------|
| `clip-vit-b32-visual.onnx` | 相似照片搜索 (图像编码) | ~338 MB |
| `clip-vit-b32-textual.onnx` | 语义文字搜索 (文本编码) | ~254 MB |
| `scrfd_500m_bnkps.onnx` | 人脸检测 | ~3 MB |
| `w600k_r50.onnx` | 人脸识别嵌入 | ~166 MB |
